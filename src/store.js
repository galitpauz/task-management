...
export const  serverAwait = (ms=2000) => {
  const img = document.createElement("IMG")
  img.setAttribute('src', '/loader.gif')
  img.setAttribute('style', 'z-index: 9999;position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%);')
  img.setAttribute('id', 'loader')
  document.getElementById('root').append(img)
  return new Promise(resolve => {
    setTimeout(resolve, ms)
  }).then(() => {
    document.getElementById('root').removeChild(document.getElementById('loader'))
  });
}
...
const cardsById = (state = {}, action) => {
  ...
  switch (action.type) {
    case "ADD_CARD": {
      ...
      return { ...state, [cardId]: { text: cardText, _id: cardId, description: cardDescription } };
    }
    ...
    case "SAVE_CARD_DATA": {
      const { cardText, cardId, cardDescription } = action.payload;
